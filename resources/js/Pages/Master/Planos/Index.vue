    ...
    <MasterLayout>
        ...
        <div v-else class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            >
                ...
                <!-- B4.4 fix · botões com rótulo (antes eram só ícones nudos sem contexto) -->
                <div class="mt-4 pt-4 border-t border-slate-100 flex items-center justify-end gap-2">
                    <button
                        ...
                        class="inline-flex items-center gap-1.5 px-3 py-2 min-h-[36px] rounded-md text-xs font-medium"
                    >
                        <svg v-if="p.is_active" class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728L5.636 5.636"/></svg>
                        <svg v-else class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        {{ p.is_active ? 'Desativar' : 'Reativar' }}
                    </button>
                    <Link
                        :href="route('master.planos.edit', p.id)"
                        class="inline-flex items-center gap-1.5 px-3 py-2 min-h-[36px] rounded-md text-xs font-medium bg-slate-100 text-slate-700 hover:bg-slate-200"
                    >
