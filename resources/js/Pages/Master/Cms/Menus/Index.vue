...
function saveAdd(menu) {
    addForm.post(route('master.cms.menus.items.store', menu.id), {
        preserveScroll: true,
        onSuccess: () => { cancelAdd(); },
    });
}
...
function saveEdit(item) {
    editForm.put(route('master.cms.menus.items.update', item.id), {
        preserveScroll: true,
...
function toggleActive(item) {
    ...
    router.put(route('master.cms.menus.items.update', item.id), {
        label: item.label,
        url: item.url,
        target: item.target,
        icon: item.icon,
...
<template>
    <Head title="Menus · CMS · Plataforma" />
    <MasterLayout>
        <template #page-title>Menus do site</template>

        <div class="mb-6">
            <h2 class="text-xl font-serif font-bold text-slate-900">Menus da landing</h2>
            <p class="mt-1 text-sm text-slate-600">
                Itens de navegação do cabeçalho e rodapé do site público.
            </p>
        </div>
