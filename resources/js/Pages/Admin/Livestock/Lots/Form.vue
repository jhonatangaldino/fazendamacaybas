        ...
        <div class="card max-w-2xl mx-auto">
            <form @submit.prevent="submit" class="card-body space-y-5">
                ...
                <div v-if="isLoteAgregado" class="rounded-xl bg-amber-50 ring-1 ring-amber-200 p-4 space-y-3">
                    ...
                    <div v-if="isEdit && lot && lot.quantidade_atual !== undefined && Number(lot.quantidade_atual) !== Number(form.quantidade_inicial)"
                         class="rounded-lg border-l-4 border-emerald-500 bg-emerald-50 px-3 py-2 flex items-center justify-between gap-3">
                        <div class="text-sm">
                            <strong class="text-emerald-900">Efetivo atual: {{ Math.round(Number(lot.quantidade_atual)) }}</strong>
                            <span class="text-emerald-700">
                                de {{ Math.round(Number(form.quantidade_inicial)) }} iniciais
                                ({{ Math.round(Number(form.quantidade_inicial) - Number(lot.quantidade_atual)) }} baixas
                                — {{ ((Number(form.quantidade_inicial) - Number(lot.quantidade_atual)) / Number(form.quantidade_inicial) * 100).toFixed(1) }}%)
                            </span>
                        </div>
                    </div>
                </div>
